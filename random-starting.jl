...
function RandomStarting(A,b,ε,q,k,x_true)
    ...
    # Ensure that \varepsilon is small enough so that I - \varepsilon * A has
    # spectral radius < 1
    ...
    for j in 0:q-1
        ...
        Y[:,1] += ε*b - ε*A*Y[:,1]
        # TODO: projected solve, compute all error values norm(x_true)
    ...
    p1 = plot(c_arr, title="Accuracy vs c", xlab="c",
    ylab="Accuracy", legend=false,linewidth=2,titlefontsize=30,
    guidefontsize=30, tickfontsize=30)
    plot(p1,size=(2000,1800))

    return x_approx 
end

function RandomStartingAccuracy(A,b,eps,q,k,x_true)
    x_approx = RandomStarting(A,b,eps,q,k,x_true)
    #@show(maximum(x_true))
    #@show(minimum(x_true))
...
ε = 0.001
q = 100
k = 99 # error just drops when k = n
x_true = A\b
...
q_values = 1:1000:max_q
k_values = 1:1000:max_k

q_accuracies = [RandomStartingAccuracy(A,b,ε,q,10000,x_true) for q in q_values]
k_accuracies = [RandomStartingAccuracy(A,b,ε,10000,k) for k in k_values]
